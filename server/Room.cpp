...
{
    std::vector<Object> plainObjs;
    plainObjs.reserve(objs.size());  // 성능 최적화 (optional)

    for (const auto& ptr : objs) {
        if (ptr) plainObjs.push_back(*ptr);  // 역참조하여 복사
    }

    DBManager::SaveRoomObjects(userID, plainObjs);
}
