...
void UQuest_Manager::Done_Process(const uint8& quest_id)
{
    ...
    if (!Quests[quest_id]->Quest_info.IsMain) {
        for (auto goal : Quests[quest_id]->Goals) {
            game_inst->SendGetItemPacket(goal.requir_item_ID, -(static_cast<int8>(goal.requir_num)));
        }
    }
